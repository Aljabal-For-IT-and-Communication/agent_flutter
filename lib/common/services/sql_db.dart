...
class SqlDbService {
  ...
  _onCreate(Database db, int version) async {
    await db.execute(
        'CREATE TABLE chat_users (id INTEGER PRIMARY KEY AUTOINCREMENT, token TEXT, cid INTEGER, name TEXT,avatar TEXT,last_msg TEXT,last_time TEXT, msg_num INTEGER)');
    print("创建数据表");
  }
  ...
  Future<int> deleteByIdChatUser(int? id) async {
    return await _database
        .delete("chat_users", where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<int> updateChatUser(ChatUserItem item) async {
    return await _database.rawUpdate(
        'UPDATE chat_users SET name = ?, avatar = ?, last_msg = ?, last_time = ?, msg_num = ? WHERE token = ?',
        [
          item.name,
          item.avatar,
          item.lastMsg,
  ...
  Future<int> clearByTokenReadMsgNum(String? token) async {
    return await _database.rawUpdate(
        'UPDATE chat_users SET msg_num = 0 WHERE token = ?', [token]);
  }
