...
class Logic{
  ...
  salePoint() async {
    try {
      ...
      if (result.code == 0) {
        ...
        context.read<RevenueBloc>().add(SalePointItemChanged(result.data!.first));
      }
    } catch (e) {
      Logger.write("${e}");
    }
  ...
  postTransformation() async{
    ...
    try {
      var result = await SalePointAPI.salePointCollectRecordList(params: entity);
      EasyLoading.dismiss();
      if (result.code == 0 && result.data!=null) {
        if (result.data!.isNotEmpty) {
          var agentCollectRecordList = state.agentCollectRecordList.toList();
          agentCollectRecordList.addAll(result.data!);
          context.read<RevenueBloc>().add(AgentCollectRecordListChanged(agentCollectRecordList));
        }
      }
